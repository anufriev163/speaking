...
export const SnippetsTab: React.FC<SnippetsTabProps> = ({ snippets, onSave }) => {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h3 className="text-xl font-semibold text-black tracking-tight mb-1">Автозамена и шаблоны</h3>
        <p className="text-xs text-neutral-500">
          Скажите кодовую фразу при диктовке (например, «мой телефон»), и программа автоматически напечатает нужный текст
        </p>
      </div>
      ...
      <form onSubmit={handleAdd} className="p-5 rounded-2xl border border-neutral-200/80 bg-white shadow-xs space-y-3.5">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-medium text-neutral-600 mb-1">Фраза при диктовке</label>
            <input
              type="text"
              value={trigger}
          ...
          <div>
            <label className="block text-xs font-medium text-neutral-600 mb-1">Пояснение (опционально)</label>
            <input
              type="text"
              value={description}
        ...
        <div>
          <label className="block text-xs font-medium text-neutral-600 mb-1">Текст или номер для вставки</label>
          <textarea
            value={replacement}
        ...
        >
          <Plus className="w-3.5 h-3.5" /> Добавить автозамену
        </button>
      </form>
